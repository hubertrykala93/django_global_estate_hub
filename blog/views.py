...
def article_categories(request, category_slug) -> django.http.response.HttpResponse:
    ...
    return render(
        request=request,
        template_name="blog/article-categories.html",
        context={
            "title": category,
            "category": category,
            "pages": blog_pagination(
                request=request,
                object_list=Article.objects.filter(category=category).order_by(
                    "-date_posted"
                ),
                per_page=8,
            ),
...
def article_tags(request, tag_slug) -> django.http.response.HttpResponse:
    ...
    return render(
        request=request,
        template_name="blog/article-tags.html",
        context={
            "title": tag,
            "pages": blog_pagination(
                request=request,
                object_list=Article.objects.filter(tags=tag).order_by("-date_posted"),
                per_page=8,
            ),
        },
    )
...
def article_details(
    request, category_slug, article_slug
) -> django.http.response.HttpResponse:
    """
    ...
    comments_counter = len(comments) - len(
        [
            obj.parent
            for obj in Comment.objects.filter(article=article, active=True)
            if obj.parent is not None
        ]
    )
    next_article = (
        Article.objects.filter(date_posted__gt=article.date_posted)
        .order_by("date_posted")
        .first()
    )
    previous_article = (
        Article.objects.filter(date_posted__lt=article.date_posted)
        .order_by("-date_posted")
        .first()
    )
    user_likes = [
        obj.comment for obj in CommentLike.objects.filter(user=request.user.id)
    ]
    user_dislikes = [
        obj.comment for obj in CommentDislike.objects.filter(user=request.user.id)
    ]
    ...
    if "search" in request.GET:
        if request.GET.get("search"):
            ...
            for keyword in keywords:
                articles.extend(
                    Article.objects.filter(title__icontains=keyword).order_by(
                        "date_posted"
                    )
                )
                articles.extend(
                    Article.objects.filter(content__icontains=keyword).order_by(
                        "-date_posted"
                    )
                )
            ...
            return render(
                request=request,
                template_name="blog/blog-results.html",
                context={
                    "title": "Blog Results",
                    "articles": articles,
                    "pages": blog_pagination(
                        request=request, object_list=articles, per_page=8
                    ),
                },
            )
...
def add_comment(
    ...
    if request.method == "POST":
        ...
        full_name, comment = [data[key] for key in data][0][0], [
            data[key] for key in data
        ][1][0]
        full_name_field, comment_field = [data[key] for key in data][0][1], [
            data[key] for key in data
        ][1][1]
        full_name_label, comment_label = [data[key] for key in data][0][2], [
            data[key] for key in data
        ][1][2]
        ...
        response = [
            {
                "valid": False if not comment else True,
                "field": comment_field,
                "message": (
        ...
        if all(validation):
            if request.user.is_anonymous:
                ...
                comment = Comment(
                    user=user, article=article, full_name=full_name, comment=comment
                )
                comment.save()
        ...
        response = {
            "valid": (
                ...
                if not comment_content
                    ...
                    False
                    if comment_content == Comment.objects.get(id=comment_id).comment
                    else True
                )
            ),
            ...
            "message": (
                ...
                if not comment_content
                    ...
                    "The comment was not edited correctly because its content did not change."
                    if comment_content == Comment.objects.get(id=comment_id).comment
                    else ""
                )
            ),
...
def delete_comment(
    ...
    if request.method == "DELETE":
        ...
        response = {
            "valid": (
                False if not Comment.objects.filter(id=comment_id).exists() else True
            ),
            "commentId": comment_id,
            "message": (
                "The comment does not exist."
                if not Comment.objects.filter(id=comment_id).exists()
                else "Your comment has been deleted."
            ),
        }
        ...
        if response["valid"]:
            ...
            comments_counter = len(
                Comment.objects.filter(article=article, active=True)
            ) - len(
                [
                    obj.parent
                    for obj in Comment.objects.filter(article=article, active=True)
            ...
            response.update({"commentsCounter": comments_counter})
...
def reply_comment(
    ...
    if request.method == "POST":
        ...
        full_name, full_name_field, full_name_label = [
            ...
            response = [
                {
                    ...
                    "message": (
                        f"The {full_name_label} field cannot be empty."
                        if not full_name
                        else (
                ...
                },
                {
                    ...
                    "field": comment_field,
                    "message": (
                        f"The {comment_label} field cannot be empty."
                        if not comment
                        else ""
                    ),
                },
        ...
        if all(validation):
            if request.user.is_anonymous:
                ...
                parent = Comment.objects.get(pk=comment_id)
                reply = Comment(
                    user=user, article=article, comment=comment, parent=parent
                )
                reply.save()
